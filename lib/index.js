...
module.exports = function createRenderer(options) {
  ...
  if (opts.paths.helpers) {
    fs.readdirSync(opts.paths.helpers).filter((file) => {
      ...
      const helperPath = path.join(opts.paths.helpers, file);

      let helper = require(helperPath);
      helper.__path = helperPath;

      return helper;
    }).filter((helper) => {
      return typeof helper === 'function' || typeof helper === 'object';
    }).forEach((helper) => {
      const helperName = path.basename(helper.__path, '.js');

      // Load helpers directly if possible; namespace them otherwise
      if (typeof helper === 'function') {
        opts.hbs.registerHelper(helperName, helper);
      } else {
        Object.keys(helper).forEach((key) => {
          if (typeof helper[key] === 'function') {
            opts.hbs.registerHelper(`${helperName}_${key}`, helper[key]);
          }
        });
      }
    });
  ...
  return async function rendererMiddleware(ctx, next) {
    ...
    ctx.render = async function render(viewName, locals) {
      let context  = Object.assign({}, ctx.state, locals),
          ...
          view     = await manager.compileTemplate(opts.paths.views, viewPath, 'view');
      ...
      if (opts.paths.layouts) {
        ...
        layout = await manager.compileTemplate(opts.paths.layouts, layoutPath, 'layout');
      }
