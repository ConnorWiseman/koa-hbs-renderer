...
module.exports = function templateManager(options) {
   ...
   * @public
   * @todo   Make obtaining the name of the template cross-platform
   */
  function compileTemplate(baseDir, filePath, type) {
    return new options.Promise((resolve, reject) => {
      let now  = timestamp();

      // The following is likely not compatible cross-platform due to the
      // different ways that file paths are normalized.
  ...
  // The cache is exposed for ease of testing.
  return { cache, compileTemplate, compileTemplates };
};
